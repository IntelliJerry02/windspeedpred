...
bp = Blueprint('admin_page_user', __name__, url_prefix='/admin_page_user')
...
def index():
    db = get_db()
    users = db.execute(
        'SELECT u.id, username, usertype, password'
        ' FROM user u '
    ).fetchall()
    return render_template('admin_page_user/admin_page_user.html', users=users)
...
def create():
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                'INSERT INTO user (username, usertype, password, id)'
                ' VALUES (?, ?, ?, ?)',
                (username, usertype, password, g.user['id'])
            )
            db.commit()
...
def get_user(id):
    ...
    # if check_author and user['author_id'] != g.user['id']:
    #     abort(403)
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                'UPDATE user SET username = ?, usertype = ?, password = ?'
                ' WHERE id = ?',
                (username, usertype, password, id)
            )
            db.commit()
            return redirect(url_for('admin_page_user.index'))

    return render_template('admin_page_user/update.html', user=user)
