##### 博客蓝图
# 博客蓝图与验证蓝图所使用的技术一样。
# 博客页面应当列出所有的帖子，允许已登录 用户创建帖子，并允许帖子作者修改和删除帖子。
...
bp = Blueprint('my_datasets', __name__, url_prefix='/my_datasets')
...
def index():
    ...
    return render_template('my_datasets/index_my_datasets.html', datasets=datasets)
...
def create():
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db.commit()
            return redirect(url_for('my_datasets.index'))

    return render_template('my_datasets/create.html')


### 更新
# update 和 delete 视图都需要通过 id 来获取一个 dataset ，并且 检查作者与登录用户是否一致。
# 为避免重复代码，可以写一个函数来获取 dataset ， 并在每个视图中调用它。
def get_dataset(id, check_author=True):
    ...
    if dataset is None:
        abort(404, f"Dataset id {id} doesn't exist.")
...
# check_author 参数的作用是函数可以用于在不检查作者的情况下获取一个 dataset 。
...
def update(id):
    ...
    return render_template('my_datasets/update.html', dataset=dataset)
...
# 要生成一个指向更新页面的 URL ，需要传递 id 参数给 url_for() ： url_for('my_datasets.update', id=dataset['id']) 。
# 前文的 index.html 文件中 同样如此。

# create 和 update 视图看上去是相似的。 
# 主要的不同之处在于 update 视图使用了一个 dataset 对象和一个 UPDATE 查询代替了一个 INSERT 查询。
...
# 默认数据集说明
@bp.route('/default_dataset_intro', methods=('GET', 'POST'))
@login_required
def default_dataset_intro():
    return render_template('my_datasets/default_dataset_intro.html')