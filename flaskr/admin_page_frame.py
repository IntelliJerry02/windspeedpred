...
bp = Blueprint('admin_page_frame', __name__, url_prefix='/admin_page_frame')
...
def index():
    db = get_db()
    users = db.execute(
        'SELECT f.id, username, usertype, password'
        ' ORDER BY created DESC'
    ).fetchall()
    return render_template('admin_page_frame/admin_page_frame.html', users=users)
