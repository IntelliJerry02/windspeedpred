...
bp = Blueprint('my_frames', __name__, url_prefix='/my_frames')
...
def index():
    ...
    return render_template('my_frames/index_my_frames.html', frames=frames)
...
def create():
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db.commit()
            return redirect(url_for('my_frames.index'))

    return render_template('my_frames/create.html')
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            db.execute(
                'UPDATE post SET title = ?, directory = ?, argument_number = ?'
                ' WHERE id = ?',
    ...
    return render_template('my_frames/update.html', frame=frame)
